...
template<typename T> T MathHelper::max (T t1, T t2)
{
	return (t1>t2)?t1:t2;
}

template<typename T> T MathHelper::min (T t1, T t2)
{
	return (t1<t2)?t1:t2;
}

template<typename T> T MathHelper::clamp (T number, T min, T max)
{
	// if min <= number <= max
